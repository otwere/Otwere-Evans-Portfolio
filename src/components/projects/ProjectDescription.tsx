        ...
        {description.split("\n").map((line, i) => {
          ...
          if (trimmed.startsWith("•") || trimmed.startsWith("-")) {
            return (
              <div
                key={i}
                className="flex items-start gap-2 pl-1 group/item hover:bg-slate-50 p-0.5 rounded-lg transition-colors -mx-1.5"
              >
                ...
                <span className="flex-1 text-[16px] leading-relaxed text-slate-600 font-normal">
          ...
          if (trimmed.endsWith(":")) {
            return (
              <h3
                key={i}
                className="font-bold text-base text-indigo-900 pt-0 pb-1"
              >
                {trimmed}
              </h3>
            );
